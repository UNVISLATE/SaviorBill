...
`GET /api/v1/user/media` — постраничный список СВОИХ загрузок (любой
статус: processing/ready/error), новые сверху; именно этот список считает
`user.media.limit`, а не Valkey-статус (см. §Конвертация ниже).
