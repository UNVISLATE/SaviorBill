"""Billing-loop: планировщик истечений услуг/платежей поверх Valkey (без своей таблицы в БД).

Очередь разделяемая между инстансами billing; атомарный claim (Lua
``ZRANGEBYSCORE``+``ZREM``) не даёт двум инстансам забрать один член.
Члены очереди детерминированы (``svc:<id>``/``pay:<id>``) — повторный засев
идемпотентен.
"""
