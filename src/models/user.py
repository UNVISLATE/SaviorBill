...
class UserModel(Base):
    ...
    def is_active(self) -> bool:
        ...
        Не используется как контроль доступа — доступ к действиям решает
        исключительно RBAC (``has_perm`` на правах текущей роли). ``banned``
        — обычная системная роль без прав по умолчанию, а не отдельный
        встроенный флаг блокировки: если явно выдать ей конкретное право,
        оно будет работать как у любой другой роли.
        """
