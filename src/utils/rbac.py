"""Иерархический RBAC (DEV.md): реестр прав, дерево и проверка доступа.

Права — дотнотация ``users.edit``. Узел-родитель наследует доступ к детям:
если у роли есть ``users`` (значение ``True``), доступны и ``users.read``, и
``users.edit``. Спец-узел ``*`` означает «всё» (суперадмин).
"""
