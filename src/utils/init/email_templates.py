...
заведены. Тело — jinja2; в контексте доступны ``user.login``, ``code`` и
``ttl_minutes``.
"""
...
просто игнорируйте это письмо.</p>""",
    ...
    (
        ...
        """<p>Здравствуйте, {{ user.login }}!</p>
<p>Вы запросили сброс пароля. Код для подтверждения:</p>
<p style="font-size:24px;font-weight:bold;letter-spacing:3px">{{ code }}</p>
<p>Код действует {{ ttl_minutes }} мин. Если вы не запрашивали сброс — никаких
действий не требуется, пароль останется прежним.</p>""",
        "Письмо с кодом сброса пароля (системное).",
    ),
)
