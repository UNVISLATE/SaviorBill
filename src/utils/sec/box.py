"""Симметричное шифрование секретов в БД (Fernet).

Используется для хранения ``client_secret`` OAuth-провайдеров. В dev-режиме,
когда ключ не задан, значения проходят насквозь (без шифрования).
"""
...
class SecBox:
    """Обёртка над Fernet с безопасным fallback при отсутствии ключа."""
    ...
    def load_or_create(path: str | Path) -> str:
        ...
        Используется, когда ``SECRETS_KEY`` не задан в окружении: ключ
