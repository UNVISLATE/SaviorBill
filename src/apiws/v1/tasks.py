"""``GET /apiws/v1/tasks/{kind}`` — realtime-хвост журнала тасков (WS).
...
4. Если сообщение не пришло за 30с, либо токен невалиден/просрочен, либо у
   аккаунта нет права ``tasks.read`` — соединение закрывается кодом 4401 без
   утечки данных (бэклог не отправляется).
...
_KINDS = ("media", "lua")
_REQUIRED_PERM = reg_perm("tasks.read")
